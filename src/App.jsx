  ...
  return (
    <div style={{
      ...
      <p style={{ margin:0, fontSize:12, color:C.textSub, textAlign:"center" }}>
        Spočítej puntíky — kolik jich je dohromady?
      </p>
    </div>
  );
}
...
function GameScreen({ mode, number, buildingType, onComplete, onBack }) {
  const STAGE_NAMES = STAGE_NAMES_BY_BUILDING[buildingType] || STAGE_NAMES_BY_BUILDING.house;
  ...
  return (
    <div style={{ position:"relative", zIndex:1, minHeight:"100vh", padding:"20px 12px" }}>
      ...
      <div className="game-grid" style={{
        ...
        gap:18, maxWidth:860, margin:"0 auto", alignItems:"start",
      }}>
        {/* LEFT: question */}
            ...
            <>
              ...
              {showHint && (
                <div style={{ marginBottom:16 }}>
                  <DotGridHint mode={mode} a={current.a} b={current.b} />
                </div>
              )}
          ...
          {feedback?.type === "correct" && (
            <div style={{ textAlign:"center", padding:"16px 0", animation:"feedbackIn 0.4s cubic-bezier(.36,1.4,.64,1)" }}>
              ...
              <div style={{ background:C.greenBg, borderRadius:14, padding:"14px 20px", color:C.green, fontWeight:700, fontSize:17, marginBottom:16 }}>
                {phase === "review"
                  ? `🧱 ${STAGE_NAMES[stage] && STAGE_NAMES[stage] + " přidáno!"}`
                  : "📘 Skvěle procvičeno!"}
              </div>
        ...
        <Card style={{ padding:"16px 12px" }}>
          <div style={{ textAlign:"center", marginBottom:8 }}>
            <span style={{ fontSize:12, fontWeight:700, color:C.textSub, letterSpacing:0.5, textTransform:"uppercase" }}>
              {buildingMeta.emoji} Tvoje {buildingMeta.label.toLowerCase()}
            </span>
          </div>
          <BuildingStage buildingType={buildingType} stage={stage} newStage={newStage} />
          <div style={{ marginTop:10 }}>
            <div style={{ height:8, background:C.accentLight, borderRadius:8, overflow:"hidden" }}>
              <div style={{
                height:"100%", borderRadius:8,
                width:`${(stage/10)*100}%`,
                background:"linear-gradient(90deg, #8DD9A2, #5BA4E0)",
                transition:"width 0.5s ease",
              }} />
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:C.textSub, marginTop:4 }}>
              <span>{stage > 0 ? STAGE_NAMES[stage] : "Zatím nic"}</span>
              <span>{stage}/10 částí</span>
            </div>
          </div>
        </Card>
      </div>
      ...
      <style>{`
        @media(max-width:580px){ .game-grid{ grid-template-columns:1fr !important; } }
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI','Nunito',system-ui,sans-serif", minHeight:"100vh", position:"relative" }}>
      ...
      {screen === "select" && (
        <SelectScreen
          onStartMult={n => startGame("mult", n)}
          onStartDiv={n  => startGame("div",  n)}
        />
      )}
      {screen === "game" && (
        <GameScreen
